  ...
  return (
    <div className="space-y-8 not-italic">
      <div className="space-y-4">
        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#003d9b]">Danh mục hàng hóa</label>
      ...
      <div className="space-y-4">
        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#003d9b]">Trạng thái tồn kho</label>
        <div className="grid grid-cols-2 gap-3">
          {statuses.map((status) => (
            ...
            >
              <span className="text-sm font-black uppercase tracking-tight">{status}</span>
              {filters.status === status && <Check size={16} strokeWidth={3} />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#003d9b]">Ngưỡng tồn tối thiểu</label>
        <input 
          type="number" 
          placeholder="Nhập số lượng..."
          value={filters.minStock}
          onChange={(e) => setFilters({...filters, minStock: e.target.value})}
          className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-1 focus:ring-[#003d9b] outline-none font-bold text-slate-900"
        />
      </div>
